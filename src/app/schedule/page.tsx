...
export default function SchedulePage() {
  ...
  const { data: tasks } = useCollection<ScheduledTask>(tasksQuery);
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="flex items-center justify-between p-3 glass rounded-xl border-white/5">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[7px] text-muted-foreground uppercase">Repetir semanal</p>
                </div>
              ...
              {formData.isRecurring && (
                <div className="flex justify-between gap-1">
                  {WEEK_DAYS.map((day) => (
                    ...
                    }))} className={cn("w-8 h-8 rounded-lg text-[8px] font-black transition-all border", formData.recurringDays.includes(day.value) ? "bg-primary text-black border-primary" : "bg-white/5 border-white/10 text-muted-foreground")}>
                      {day.label}
                    </button>
                  ))}
                </div>
        ...
        {days.map((day) => {
          ...
          return (
            <button key={day.toString()} onClick={() => setSelectedDate(day)} className={cn("flex-shrink-0 w-16 py-3 rounded-2xl flex flex-col items-center gap-1 transition-all border", isSelected ? "bg-primary text-black border-primary scale-105 shadow-lg" : "glass border-white/5")}>
              <span className={cn("text-[7px] uppercase font-black tracking-widest", isSelected ? "text-black/60" : "text-muted-foreground/50")}>{format(day, 'EEE', { locale: es })}</span>
        ...
        <div className="space-y-8">
          <AnimatePresence mode="popLayout">
            {dailyTasks.length > 0 ? (
                    ...
                    <div className="flex flex-col md:flex-row md:items-start gap-3 md:gap-12">
                      ...
                      <div className={cn("flex-1 glass p-4 md:p-6 rounded-[1.5rem] flex flex-col gap-4 border-l-4 transition-all relative overflow-hidden group", task.priority === 'alta' ? 'border-l-red-500' : isActive ? 'border-l-primary' : 'border-l-white/10')}>
                        <div className="absolute top-0 right-0 p-3 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity z-10 flex gap-1.5">
                           ...
                           <Button variant="ghost" size="icon" onClick={() => { if(confirm("¿Eliminar?")) deleteDocumentNonBlocking(doc(firestore, "users", user.uid, "tasks", task.id)); }} className="h-8 w-8 rounded-lg bg-black/40"><Trash2 className="w-3.5 h-3.5" /></Button>
                        </div>
                        <div className="space-y-1.5">
                            <h4 className="text-base md:text-xl font-black tracking-tighter text-white pr-12">{task.title}</h4>
                            <div className="flex gap-2">
                              {task.isRecurring && <Badge className="bg-primary/10 text-primary border-primary/20 text-[7px] font-black px-1.5 py-0.5">RUTINA</Badge>}
                              <Badge variant="outline" className="text-[7px] font-black uppercase px-1.5 py-0.5">{task.priority || 'media'}</Badge>
                            </div>
                        </div>
                        {isActive && (
                          <div className="space-y-1.5 bg-white/5 p-3 rounded-lg border border-white/5">
                            <div className="flex justify-between text-[7px] font-black text-primary uppercase"><span>Sistema Activo</span><span>{progress}%</span></div>
                            <Progress value={progress} className="h-1" />
                          </div>
                        )}
                        <div className="flex items-center gap-4 text-[8px] text-white/20 uppercase font-black tracking-widest"><Clock className="w-3 h-3 text-primary" /> {format(parseISO(task.scheduledStartTime), "HH:mm")} - {task.scheduledEndTime ? format(parseISO(task.scheduledEndTime), "HH:mm") : '...'}</div>
                      </div>
                    </div>
                  </motion.div>
                );
              ...
              <div className="flex flex-col items-center justify-center py-20 opacity-5"><Inbox className="w-12 h-12 mb-2" /><p className="text-[8px] font-black uppercase tracking-[0.4em]">Sin Bloques</p></div>
            )}
          </AnimatePresence>
        </div>
      </div>

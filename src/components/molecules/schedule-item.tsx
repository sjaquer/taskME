      ...
      <div className="flex flex-col md:flex-row md:items-start gap-3 md:gap-12">
        ...
        <div
          className={cn(
            ...
            routine.priority === 'alta' ? 'border-l-red-500' : isActive ? 'border-l-primary' : 'border-l-white/10'
          )}
        ...
        >
          <div className="absolute top-0 right-0 p-3 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity z-10 flex gap-1.5">
            <Button variant="ghost" size="icon" onClick={() => onEdit(routine)} className="h-8 w-8 rounded-lg bg-[#050505]/60">
              <Edit3 className="w-3.5 h-3.5" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => onDelete(routine.id)} className="h-8 w-8 rounded-lg bg-[#050505]/60">
              <Trash2 className="w-3.5 h-3.5" />
            </Button>
          </div>
          <div className="space-y-1.5">
            <h4 className="text-base md:text-xl font-black tracking-tighter text-white pr-12">{routine.title}</h4>
